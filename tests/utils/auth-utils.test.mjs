...
describe("auth-utils", () => {
  ...
  // CLEAR STORED ACCESS TOKEN TESTS
  describe("clearStoredAccessToken", () => {
    test("should clear access token from config file", async () => {
      existsSyncSpy.mockReturnValue(true);
      readFileSpy.mockResolvedValue(
        "example.com:\n  DOC_DISCUSS_KIT_ACCESS_TOKEN: test-token\n  DOC_DISCUSS_KIT_URL: https://example.com",
      );
      parseSpy.mockReturnValue({
        "example.com": {
          DOC_DISCUSS_KIT_ACCESS_TOKEN: "test-token",
      ...
      await clearStoredAccessToken("https://example.com");
    ...
    test("should remove entire hostname entry when only token exists", async () => {
      existsSyncSpy.mockReturnValue(true);
    ...
    test("should preserve other hostnames in config", async () => {
      existsSyncSpy.mockReturnValue(true);
      readFileSpy.mockResolvedValue("config content");
      parseSpy.mockReturnValue({
        "example.com": {
          DOC_DISCUSS_KIT_ACCESS_TOKEN: "test-token",
          DOC_DISCUSS_KIT_URL: "https://example.com",
    ...
    test("should handle non-existent hostname in config", async () => {
      existsSyncSpy.mockReturnValue(true);
      readFileSpy.mockResolvedValue("other.com:\n  token: other-token");
      parseSpy.mockReturnValue({
        "other.com": {
      ...
      // Should not modify the config since hostname doesn't exist
    ...
    test("should handle file read errors gracefully", async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        "Failed to clear stored access token:",
        "File read error",
      );
      expect(writeFileSpy).not.toHaveBeenCalled();
    });
    ...
    test("should handle different URL formats correctly", async () => {
      ...
      for (const url of urls) {
        ...
        await clearStoredAccessToken(url);

        // All should result in same hostname "example.com"
        expect(stringifySpy).toHaveBeenCalledWith({});
      }
    });

    test("should handle subdomain URLs correctly", async () => {
      existsSyncSpy.mockReturnValue(true);
      readFileSpy.mockResolvedValue("config content");
      parseSpy.mockReturnValue({
        "sub.example.com": {
      ...
      await clearStoredAccessToken("https://sub.example.com");
    ...
    test("should handle invalid URL gracefully", async () => {
      await expect(clearStoredAccessToken("invalid-url")).rejects.toThrow();
    });
  });
});

...
`.trim()
  ...
  describe('isRemoteTextFile', () => {
    ...
    describe('isRemoteFileAvailable', () => {
      test('should detect http and https URLs', async () => {
        ...
        try {
          const result = await isRemoteFileAvailable('https://www.arcblock.io/.well-known/glossary.txt')
          expect(result).toBe(true)
        } finally {
          globalThis.fetch = originalFetch
        }
  ...
  describe('findInvalidSourcePaths', () => {
    ...
    test('skips paths starting with !', async () => {
      const result = await findInvalidSourcePaths(['!src/file.js', '!node_modules'], ['node_modules/**'])
    ...
    test('handles mixed valid and invalid paths', async () => {
      ...
      try {
        ...
        const result = await findInvalidSourcePaths(
          [validDir, excludedDir, testFile, nonExistent, 'http://example.com', '!excluded', 'node_modules/**/*.js'],
          ['**/node_modules/**'],
        )

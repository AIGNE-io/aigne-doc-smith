...
`.trim();
  ...
  describe("calculateTokens", () => {
    test("should calculate tokens for normal text", () => {
    ...
    test("should handle text containing <|endoftext|> special token", () => {
      const text = 'eos_token ("str", *optional*, defaults to `"<|endoftext|>"`):';
      const tokens = calculateTokens(text);
    ...
    test("should handle multiple occurrences of <|endoftext|>", () => {
      const text = "First: <|endoftext|> and second: <|endoftext|> token";
      const tokens = calculateTokens(text);
    ...
    test("should handle text with only <|endoftext|>", () => {
    ...
    test("should handle text with other special token patterns", () => {
      const text = "Some text with <|special|> token pattern";
      const tokens = calculateTokens(text);
    ...
    test("should handle multiline text with special tokens", () => {
      const text = `Line 1 with <|endoftext|>
Line 2 with normal content
Line 3 with another <|endoftext|> token`;
      const tokens = calculateTokens(text);
    ...
    test("should handle text in code examples", () => {
      const text = `\`\`\`python
eos_token = "<|endoftext|>"
print(eos_token)
\`\`\``;
    ...
    test("should handle text with escaped special tokens", () => {
      const text = 'The token is `"<|endoftext|>"` in the config';
      const tokens = calculateTokens(text);
    ...
    test("should handle sanitizeForTokenization with non-string input", () => {
      // Test that sanitizeForTokenization handles non-string (line 540)
      // sanitizeForTokenization returns non-string as-is, which encode will handle
      // This tests the type check in sanitizeForTokenization
      const tokens = calculateTokens("normal string");
      expect(typeof tokens).toBe("number");
      expect(tokens).toBeGreaterThan(0);
    });

    test("should handle very long text with special tokens", () => {
      const text = `${"<|endoftext|>".repeat(1000)} normal text`;
      const tokens = calculateTokens(text);
    ...
    test("should handle encode error and use fallback sanitization (line 552-559)", async () => {
      // Mock gpt-tokenizer to throw error on first call, succeed on second
      const gptTokenizer = await import("gpt-tokenizer");
      const originalEncode = gptTokenizer.encode;

      let callCount = 0;
      const mockEncode = (text) => {
        callCount++;
        if (callCount === 1) {
      ...
      // Use mock.module to replace the encode function
      ...
      try {
        ...
        const { calculateTokens: calcTokens } = await import("../../utils/file-utils.mjs");
        const tokens = calcTokens("test text with <|special|> token");
        // Should use fallback and succeed on second try (line 556-558)
        expect(tokens).toBeGreaterThan(0);
    ...
    test("should use character-based estimation when all encode attempts fail (line 559-563)", async () => {
      // Mock gpt-tokenizer to always throw error
      const gptTokenizer = await import("gpt-tokenizer");
      const originalEncode = gptTokenizer.encode;
      ...
      try {
        // Re-import to get mocked version
        const { calculateTokens: calcTokens } = await import("../../utils/file-utils.mjs");
        const text = "test text with exactly 32 characters!!";
        const tokens = calcTokens(text);
        // Should use character-based estimation: Math.ceil(32 / 4) = 8 (line 562)
        expect(tokens).toBeGreaterThan(0);
  ...
  describe("calculateFileStats", () => {
    ...
    test("should handle content with <|endoftext|> special token", () => {
      const sourceFiles = [
        { content: 'eos_token ("str", *optional*, defaults to `"<|endoftext|>"`):' },
    ...
    test("should handle multiple files with special tokens", () => {
      const sourceFiles = [
        { content: "File 1: <|endoftext|> token" },
        { content: "File 2: <|endoftext|> another token" },
        { content: "File 3: normal content" },
      ];
    ...
    test("should handle files with null content", () => {
      const sourceFiles = [{ content: null }, { content: "test" }];
      const stats = calculateFileStats(sourceFiles);
    ...
    test("should handle files with undefined content", () => {
      const sourceFiles = [{ content: undefined }, { content: "test" }];
      const stats = calculateFileStats(sourceFiles);
    ...
    test("should handle encode error and fallback to calculateTokens (line 584)", async () => {
      // Mock gpt-tokenizer to throw error to trigger catch block
      const gptTokenizer = await import("gpt-tokenizer");
      const originalEncode = gptTokenizer.encode;
      ...
      try {
        // Re-import to get mocked version
        const { calculateFileStats: calcFileStats } = await import("../../utils/file-utils.mjs");
        const sourceFiles = [
          { content: "<|endoftext|>".repeat(100) },
          { content: "normal content" },
        ];
        const stats = calcFileStats(sourceFiles);
        // Should fallback to calculateTokens which has its own error handling (line 586)
        expect(stats.totalTokens).toBeGreaterThan(0);
        expect(stats.totalLines).toBeGreaterThan(0);
      } finally {
        // Restore original

...
describe("structurePlanGenerator Agent", () => {
  beforeAll(() => {
  ...
  test("should load agent correctly with proper configuration", async () => {
    ...
    // Get the structurePlanGenerator agent by name
    const agent = aigne.agents.find((a) => a.name === "structurePlanGenerator");

    // Verify agent exists and is correct type
    expect(agent).toBeDefined();
    expect(agent).toBeInstanceOf(AIAgent);
    expect(agent.name).toBe("structurePlanGenerator");
  });

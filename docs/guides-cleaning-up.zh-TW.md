...
| Target | Description |
| :--- | :--- |
| **`generatedDocs`** | 允許您從輸出目錄（例如 `./docs`）中選取並刪除特定的已建立文件。整體文件結構將被保留。 |
| **`documentStructure`** | 刪除所有已建立的文件檔案和結構計畫檔案（例如 `.aigne/doc-smith/output/structure-plan.json`）。 |
...
| **`mediaDescription`** | 刪除專案媒體檔案的 AI 建立描述快取（例如 `.aigne/doc-smith/media-description.yaml`）。這些描述將在下次需要時自動重新建立。 |
...
此指令會刪除已建立的文件檔案，但保留 `structure-plan.json` 檔案，讓您之後可以重新建立內容。
...
此指令會執行更徹底的清理，移除所有已建立的文件、結構計畫以及主要組態檔。

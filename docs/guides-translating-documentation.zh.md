...
将文档翻译成多种语言对于触达全球受众至关重要。本指南提供了使用 `aigne doc translate` 命令将内容翻译成 12 种支持语言之一的详细步骤，确保您的文档易于各地用户访问和理解。

## 翻译流程概述

翻译文档的主要命令是 `aigne doc translate`。该命令可以在两种模式下执行：交互式或非交互式（使用命令行标志）。两种方法都设计得简单直接，让您能够高效地管理单语言或多语言的翻译工作。

### 交互模式

如需引导式体验，请在不带任何参数的情况下运行该命令。对于不熟悉此翻译功能或偏好分步操作的用户，推荐使用此方法。
...
1.  首先，系统会提示您从项目中所有可用的文档文件列表中选择要翻译的具体文档。
...
确认选择后，DocSmith 将开始把每个文档翻译成您选择的每一种语言。
...
<x-field-group>
  ...
  <x-field data-name="--glossary" data-type="string" data-required="false">
    <x-field-desc markdown>术语表文件的路径（例如 `@/path/to/glossary.md`）。该文件有助于在所有翻译中为特定术语保持一致性。</x-field-desc>
  </x-field>
  <x-field data-name="--feedback" data-type="string" data-required="false">
    <x-field-desc markdown>向 AI 提供具体指令以指导其翻译风格（例如，“使用正式语气，并保持技术术语为英文”）。此反馈也会记录在文档的历史记录中，以备将来参考。</x-field-desc>
  </x-field>
</x-field-group>
...
2.  **使用术语表和风格反馈进行翻译：**
    ...
    要将 `overview.md` 翻译成德语，同时确保术语一致并采用正式语气，您可以加入 `--glossary` 和 `--feedback` 选项：
    ```bash icon=lucide:terminal
...
本指南介绍了如何使用 `aigne doc translate` 命令，使您的文档能够触达全球受众。您可以使用交互模式进行引导式操作，或使用命令行选项实现自动化和精确控制。

文档翻译完成后，下一步就是发布它们。有关此流程的详细说明，请参阅[发布您的文档](./guides-publishing-your-docs.md)指南。
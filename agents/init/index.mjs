...
export default async function init(input, options) {
  ...
  options.context.userContext.thinkingEffort = config.thinking?.effort || DEFAULT_THINKING_EFFORT_LEVEL
...
async function _init(
  { outputPath = '.aigne/doc-smith', fileName = 'config.yaml', skipIfExists = false, appUrl, checkOnly = false },
  options,
) {
  ...
  const mappedPurpose = prioritizedPurposes.find((purpose) => PURPOSE_TO_KNOWLEDGE_MAPPING[purpose])
  ...
  const availableTranslationLanguages = SUPPORTED_LANGUAGES.filter((lang) => lang.code !== primaryLanguageChoice)
  ...
  try {
    ...
    console.error(`❌ Sorry, I encountered an error while saving your configuration file: ${error.message}`)
    return {
...
\n`
  ...
  yaml += '# glossary: "@glossary.md"  # Path to a markdown file containing glossary definitions.\n\n'

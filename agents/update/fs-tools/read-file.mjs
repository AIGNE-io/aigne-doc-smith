...
async function readFileContent(filePath, offset = 0, limit = null, encoding = 'utf8') {
  ...
  if (mimeType.startsWith('image/') || mimeType === 'application/pdf' || fileSize > 10 * 1024 * 1024) {
    return {
...
export default async function readFile({
  ...
  try {
    ...
    if (fileResult.isTruncated) {
      ...
      result.message = `Content truncated. Showing lines ${start}-${end} of ${fileResult.lineCount} total lines. ` +

...
2. 如果检测到 `.aigne/doc-smith` 目录存在，执行版本检查：
   - 读取 `config.yaml` 文件
   - 检查是否有 `mode` 字段且值在 `WORKSPACE_MODES`（project/standalone）中
   - 如果是新版本配置，正常继续
   - 如果不是新版本（无 mode 字段或值无效），执行备份迁移
3. 如果已初始化（新版本 config.yaml 存在），跳过初始化，直接返回
4. 如果未初始化，根据检测到的模式执行对应的初始化流程

### 流程 0：老版本备份迁移

**触发条件**：`.aigne/doc-smith` 目录存在，但 `config.yaml` 不是新版本格式

**版本判断标准**：
- 新版本：config.yaml 中有 `mode` 字段，且值为 `project` 或 `standalone`
- 老版本：config.yaml 不存在、无 `mode` 字段、或 `mode` 值不在有效范围内

**步骤**：
1. 将 `.aigne/doc-smith` 目录重命名为 `.aigne/doc-smith-bak`
2. 输出提示信息，告知用户老版本已备份
3. 继续执行正常的初始化流程（流程 A）
...
检测现有 config.yaml 是否为新版本格式：
- 读取并解析 config.yaml 文件
- 检查 `mode` 字段是否存在
- 验证 `mode` 值是否在 `WORKSPACE_MODES`（project/standalone）中
- 返回是否为新版本的判断结果

### 3. 老版本备份迁移

当检测到老版本配置时：
- 将 `.aigne/doc-smith` 目录重命名为 `.aigne/doc-smith-bak`
- 保留用户之前的工作内容
- 输出迁移提示信息
...
4. **备份目录已存在**：`.aigne/doc-smith-bak` 目录已存在
5. **目录重命名失败**：无法重命名老版本目录
...
4. **备份目录已存在**：提示用户手动处理备份目录，阻断初始化流程
5. **目录重命名失败**：输出错误信息，提示检查权限或磁盘空间

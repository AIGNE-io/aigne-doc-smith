...
您是否曾經希望您的 AI 助理能記住您的指示？本指南將說明如何檢視、移除和切換已儲存的文件建立偏好設定，讓您能精細地控制 AI 的輸出，並確保其始終遵循您專案的特定風格。

當您生成或更新文件時，可以使用 `--feedback` 旗標提供回饋。此回饋會被儲存為「偏好設定」，以便在未來的會話中重複使用，確保 AI 與您先前的指示保持一致性。`aigne doc prefs` 指令提供了一種直接管理這些已儲存偏好設定的方法。

本指南詳細介紹如何列出、移除和切換已儲存偏好設定的啟用狀態。下圖說明了管理這些偏好設定的工作流程。

<!-- DIAGRAM_IMAGE_START:guide:4:3:1765281225 -->
![管理偏好設定](assets/diagram/managing-preferences-diagram-0.zh-TW.jpg)
<!-- DIAGRAM_IMAGE_END -->
...
若要從列表中選擇偏好設定，請執行不帶任何 ID 的指令。這將開啟一個互動式提示，您可以在其中勾選希望刪除的項目。

```bash icon=lucide:terminal
aigne doc prefs --remove
```

一個核對清單將會出現，讓您選擇一個或多個偏好設定。這是確保您移除正確項目的建議方法。

### 直接模式

如果您已經知道要移除的偏好設定的唯一 ID，可以使用 `--id` 旗標指定它們。如果您確定要刪除哪些項目，這種方式會更快。
...
執行不帶 ID 的指令將啟動一個互動式核對清單，類似於移除指令。

```bash icon=lucide:terminal
aigne doc prefs --toggle
```

您可以選擇希望啟用或停用的偏好設定。狀態圖示（`🟢`/`⚪`）將會更新以反映新的狀態。
...
透過管理您的偏好設定，您可以對文件建立過程保持精細的控制，確保輸出始終符合您專案的特定要求和風格。
...
需要快速修改或徹底翻新文件嗎？本指南將說明如何使用 `update` 指令來修改現有文件，無論是整合新的回饋、反映程式碼變更，還是從頭開始。此工具提供了用於引導式變更的互動模式，以及用於快速命令行更新的直接模式。
...
1.  開始互動式更新流程：
    ...
    aigne doc update
    ```
2.  您將會看到專案文件的列表。使用方向鍵移動，使用空格鍵選擇一個或多個檔案，然後按 `Enter` 確認您的選擇。

    ![顯示互動式提示的螢幕截圖，使用者可以從列表中選擇要更新的文件。](../../../assets/screenshots/doc-update.png)

接下來的流程將取決於您是選擇單一文件還是多個文件。

### 精修單一文件

當您選擇一份文件時，將進入一個專為精確調整設計的迭代精修循環。您將看到一組選項：

*   **View document**：直接在您的終端機中顯示文件內容的當前版本。
*   **Give feedback**：提示您輸入文字，描述您想要的變更。例如，「為非技術使用者簡化引言」或「為驗證函式新增一個程式碼範例」。
*   **Done**：退出精修循環並儲存文件的最新版本。

在您提交回饋後，工具會重新生成文件內容。然後您可以查看新版本並提供更多回饋，重複此循環，直到您對結果感到滿意為止。

### 批次更新多個文件

如果您選擇了兩個或更多文件，工具將執行批次更新。系統會提示您提供單一的指示或回饋。此回饋將同時應用於所有選定的文件。這種方法對於在您的文件中進行一致性變更非常有效，例如更新一個重複出現的章節或統一術語。

## 使用命令行旗標進行直接更新

當您已經知道需要進行哪些變更時，可以使用命令行旗標直接更新文件，從而繞過互動式提示。
...
您可以多次指定 `--docs` 旗標，以便在一個指令中將相同的回饋應用於多個文件。
...
若要捨棄文件的當前版本並從原始碼重新生成它，請使用 `--reset` 旗標。此操作會告知工具忽略現有內容，並從頭開始建立文件。
...
| 參數 | 說明 | 必填 |
| :--- | :--- | :--- |
| `--docs` | 指定要更新的文件路徑。可多次使用。 | 選填 |
| `--feedback` | 提供對指定文件進行變更的文字指示。 | 選填 |
| `--reset` | 一個布林旗標，若存在，則會從頭開始重新建立文件，忽略現有內容。 | 選填 |
| `--glossary` | 指定詞彙表檔案的路徑（`@/path/to/glossary.md`），以確保術語的一致性。 | 選填 |

---

透過使用 `update`、`add-document` 和 `remove-document` 指令，您可以管理文件的完整生命週期，使其與您的專案開發保持一致。有關首次建立文件的資訊，請參閱 [建立文件](./guides-generating-documentation.md) 指南。
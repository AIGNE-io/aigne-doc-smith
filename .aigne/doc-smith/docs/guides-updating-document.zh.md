...
需要快速更改或彻底修改文档？本指南将说明如何使用 `update` 命令来修改现有文档，无论您是采纳新反馈、反映代码变更，还是从头开始。该工具提供了用于引导式更改的交互模式和用于快速命令行更新的直接模式。
...
2.  您将看到项目文档的列表。使用箭头键移动，使用空格键选择一个或多个文件，然后按 `Enter` 键确认您的选择。
    ...
    ![显示交互式提示的屏幕截图，用户可以从列表中选择要更新的文档。](../../../assets/screenshots/doc-update.png)

接下来的流程取决于您是选择单个文档还是多个文档。

### 优化单个文档

当您选择一个文档时，您将进入一个为进行精确调整而设计的迭代优化循环。您将看到一组选项：

*   **View document**：直接在您的终端中显示文档内容的当前版本。
*   **Give feedback**：提示您输入描述所需更改的文本。例如，“为非技术用户简化引言”或“为身份验证函数添加一个代码示例”。
...
如果您选择两个或更多文档，该工具将执行批量更新。系统会提示您提供单个指令或反馈。然后，此反馈将同时应用于所有选定的文档。此方法对于在整个文档中进行一致性更改非常高效，例如更新重复出现的部分或规范术语。
...
您可以多次指定 `--docs` 标志，以在一个命令中将相同的反馈应用于多个文档。
...
要丢弃文档的当前版本并从源代码重新生成它，请使用 `--reset` 标志。此操作会告知工具忽略现有内容并从头开始创建文档。
...
虽然 `update` 命令可以修改现有内容，但您可能还需要添加新文档或移除不再相关的文档。对于这些任务，DocSmith 提供了专门的命令：
...
| `--glossary` | 指定词汇表文件（`@/path/to/glossary.md`）的路径，以确保术语一致。 | 可选 |
